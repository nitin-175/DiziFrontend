...
const Cart = () => {
  ...
  useEffect(() => {
    // Trigger the slide-in effect after mount
    if (cartRef.current) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 bg-[#facc15] bg-opacity-90 z-[1000]" />
      
      <div
        ref={cartRef}
        className="fixed top-0 right-0 h-full w-full sm:w-1/2 bg-gray-50 z-[1001] shadow-2xl transform transition-transform duration-500 translate-x-full overflow-y-auto"
        style={{ maxWidth: 600 }}
      >
        <div className="p-6">
          ...
          {cartItems.length === 0 ? (
            <div className="text-center py-16">
              <img src="/assets/empty-cart.jpeg" alt="Empty Cart" className="mx-auto w-32 h-32 mb-4" />
              <h2 className="text-xl font-semibold mb-2">Your cart is empty</h2>
              <p className="text-gray-500 mb-6">Looks like you haven't added anything to your cart yet.</p>
              <a href="/category" className="inline-block bg-orange-500 text-white px-6 py-2 rounded-lg font-semibold hover:bg-orange-600 transition">Continue Shopping</a>
            </div>
          ) : (
            <>
              {cartItems.map((item, idx) => (
                <div key={item.id} className="flex items-center justify-between mb-6 pb-6 border-b">
                  ...
                  <div className="flex items-center space-x-6">
                    <span className="px-3 py-1 border rounded-md bg-gray-100">{item.quantity || 1}</span>
              ...
              <div className="mt-8 bg-white p-6 rounded-lg shadow-sm">
                ...
                <div className="mt-6">
                  ...
                  <a href="/category" className="block text-center text-sm bg-orange-500 hover:bg-orange-600 text-white font-semibold mt-4 py-2 rounded-lg transition">Continue Shopping</a>
                </div>
